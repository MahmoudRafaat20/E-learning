...
const AdminLessons = () => {
  ...
  const { lessons, loading, error, refetch, deleteLesson, updateLesson, isAdmin } = useAdminLessons(adminFilters);
...
const handleUpdate = async (lessonId, updatedData) => {
  try {
    ...
    if (result.success) {
      ...
      // refresh البيانات من الـ API بعد تحديث ناجح
      setTimeout(() => {
        refetch();
      }, 500);
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-blue-700 md:text-4xl"> All Lessons</h1>
            <p className="text-gray-600 mt-1">Manage lessons across all class levels</p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={() => {
                // Add new lesson functionality here
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border border-b-gray-400 border-t-transparent border-x-transparent">
                  <tr>
                    <th className="px-6 py-4 text-left  font-bold text-violet-700 uppercase tracking-wider">Lesson</th>
                    <th className="px-6 py-4 text-left  font-bold text-violet-700 uppercase tracking-wider">Class Level</th>
                    <th className="px-6 py-4 text-left  font-bold text-violet-700 uppercase tracking-wider">Price</th>
                    <th className="px-6 py-4 text-left  font-bold text-violet-700 uppercase tracking-wider">Type</th>
                    <th className="px-6 py-4 text-left  font-bold text-violet-700 uppercase tracking-wider">Schedule</th>
                    <th className="px-6 py-4 text-left  font-bold text-violet-700 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredLessons.map((lesson) => (
                    <tr key={lesson._id} className="hover:bg-gray-50 transition-colors border border-b-gray-400 border-t-transparent border-x-transparent">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">{lesson.price} $</td>
                      <td className="px-6 py-4">
